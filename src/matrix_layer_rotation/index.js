function matrixRotation(matrix, r) {
    ...
    resultado = Array.from(Array(m), () => new Array(n));
...
for(i = 0; i < m; i++){
    for(j = 0; j < n; j++){
        if ((j >= i) && (j < n - i) && (i < numCadenas)){
            resultado[i][j] = matrix[matrix3[i][j-i][0]][matrix3[i][j-i][1]];
        } else if ((j >= ((m-1) - i)) && (j < (n-((m-1)-i))) & (((m-1)-i) < numCadenas)){
           resultado[i][j] = matrix[matrix3[(m-1)-i][( (m-2*((m-1)-i)) + (n-2*((m-1)-i)-2) +  ((n-1)-j-((m-1)-i)) )][0]][matrix3[(m-1)-i][( (m-2*((m-1)-i)) + (n-2*((m-1)-i)-2) +  ((n-1)-j-((m-1)-i)) )][1]];
        } else if (( j < i ) && (j < numCadenas)){
            resultado[i][j] = matrix[matrix3[j][( 2*(n-2*(j)) + (m-2*(j)-3) + ((m-1)-i-j) )][0]][matrix3[j][( 2*(n-2*(j)) + (m-2*(j)-3) + ((m-1)-i-j) )][1]];
        } else {
            resultado[i][j] = matrix[matrix3[(n-1-j)][ ((n-2*(n-1-j)) + (i - (n-1-j)) - 1) ][0]][matrix3[(n-1-j)][ ((n-2*(n-1-j)) + (i - (n-1-j)) - 1) ][1]];
        }
    }
}
